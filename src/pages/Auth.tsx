...
const signUpSchema = z.object({
  ...
  age: z.coerce.number().int().min(10, 'Usia minimal 10 tahun').max(60, 'Usia maksimal 60 tahun'),
  phone: z.string().min(8, 'No. HP minimal 8 digit'),
  email: z.string().email({ message: 'Format email tidak valid.' }),
  password: z.string().min(8, { message: 'Password minimal 8 karakter.' }),
  gestationalAge: z.coerce.number().int().min(1, 'Usia kehamilan minimal 1 minggu').max(45, 'Usia kehamilan maksimal 45 minggu'),
...
function FieldIcon({ icon }: { icon: React.ReactNode }) {
  return <div className="absolute inset-y-0 left-3 flex items-center text-rose-400">{icon}</div>;
}
...
export default function Auth() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      toast({ title: 'Gagal Masuk Google', description: mapAuthError(e?.message ?? ''), variant: 'destructive' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSignUp = async (values: SignUpValues) => {
    ...
    try {
      ...
      toast({ title: 'Pendaftaran Gagal', description: mapAuthError(error.message), variant: 'destructive' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const renderForm = () => {
      ...
      return (
        <>
          <div className="text-center">
            <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Selamat Datang ♥</h2>
            <p className="text-slate-600 dark:text-slate-300">Masuk ke akun CalmyCare Anda</p>
          </div>
          <Form {...signInForm}>
            <form onSubmit={signInForm.handleSubmit(handleSignIn)} className="space-y-5 pt-6">
                  ...
                  <FormItem>
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input placeholder="nama@email.com" className="h-12 rounded-xl pl-10 shadow-sm" {...field} />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormItem>
                    <FormControl>
                      <div className="relative">
                        ...
                        <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-9 w-9" onClick={() => setShowPassword((s) => !s)} aria-label={showPassword ? 'Sembunyikan' : 'Tampilkan'}>
              ...
              />
              <Button type="submit" className="w-full h-12 rounded-full bg-rose-400 hover:bg-rose-500 text-white shadow-md text-base" disabled={isLoading}>
                {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Masuk
              </Button>
              <Button type="button" onClick={handleGoogleSignIn} className="w-full h-12 rounded-full bg-white dark:bg-transparent border dark:border-white/20 shadow-sm" disabled={isLoading}>
                <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="" className="h-5 w-5 mr-3" /> Masuk dengan Google
              </Button>
            </form>
          </Form>
          <p className="text-center text-sm text-slate-600 dark:text-slate-300 pt-6">
            Belum punya akun?{' '}
            <button onClick={() => setTab('signup')} className="font-semibold text-rose-500 hover:underline">
              Daftar sekarang
            </button>
          </p>
        </>
    ...
    return (
      <>
        <div className="text-center">
          <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Buat Akun Baru</h2>
          <p className="text-slate-600 dark:text-slate-300">Isi data untuk mendaftar</p>
        </div>
        <Form {...signUpForm}>
                  ...
                  <FormItem>
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input type="number" placeholder="Usia" className="h-12 rounded-xl pl-10 shadow-sm" {...field} />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormItem>
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input placeholder="No. Telepon" className="h-12 rounded-xl pl-10 shadow-sm" {...field} />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <div className="relative">
                      <FieldIcon icon={<Lock size={18} />} />
                      <Input type={showPassword ? 'text' : 'password'} placeholder="Password (min. 8 karakter)" className="h-12 rounded-xl pl-10 pr-10 shadow-sm" {...field} />
                      <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-9 w-9" onClick={() => setShowPassword((s) => !s)} aria-label={showPassword ? 'Sembunyikan' : 'Tampilkan'}>
                        {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                      </Button>
                    </div>
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <div className="relative">
                        <FieldIcon icon={<Calendar size={18} />} />
                        <Input type="number" placeholder="Usia Kehamilan (minggu)" className="h-12 rounded-xl pl-10 shadow-sm" {...field} />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                ...
                render={({ field }) => (
                  <FormItem>
                    <div className="relative">
                      ...
                      <FormControl>
                        <Input placeholder="Pekerjaan" className="h-12 rounded-xl pl-10 shadow-sm" {...field} />
                      </FormControl>
                    </div>
                    <FormMessage />
                  </FormItem>
        ...
        <p className="text-center text-sm text-slate-600 dark:text-slate-300 pt-6">
          Sudah punya akun?{' '}
          <button onClick={() => setTab('signin')} className="font-semibold text-rose-500 hover:underline">
            Masuk
          </button>
        </p>
      </>
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center gap-2.5 mb-2">
            ...
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-800 dark:text-slate-100">CalmyCare</h1>
          </div>
          <p className="text-sm text-slate-600 dark:text-slate-300">Skrining kecemasan ibu hamil primigravida</p>
        </div>
        <div className="rounded-3xl shadow-xl bg-white/80 dark:bg-white/10 backdrop-blur-md p-6 sm:p-8">{renderForm()}</div>
        <div className="text-center mt-6">
          <Button variant="link" onClick={() => navigate('/')} className="text-slate-600 dark:text-slate-300">
            Kembali ke Beranda
          </Button>
        </div>
      </div>
